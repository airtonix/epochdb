...
"<div data-ng-controller=\"ItemDetailController\" class=\"large-6 columns large-centered\"><div class=\"epoch-item-description\">{{ thing.description }}</div><div data-collection=\"{{ thing.requiredComponents }}\" data-ng-show=\"selected\" data-tab-pane=\"requiredComponents\" data-tab-name=\"mats\" class=\"content\"></div></div>");
}]);

angular.module("/epochdb/js/templates/home.html", []).run(["$templateCache", function($templateCache) {
$templateCache.put("/epochdb/js/templates/home.html",
"<div class=\"hero\"><div class=\"cell\"><div class=\"columns\"><h2><a href=\"{{WinterSmith.url}}\">{{WinterSmith.name}}<small>{{WinterSmith.description}}</small></a></h2><search-form data-change-behaviour=\"submit\"></search-form></div></div></div>");
}]);
...
$templateCache.put("/epochdb/js/templates/page.html",
"<div data-ng-include=\"PageUrl\" class=\"columns large-12\"></div>");
}]);
...
"<div class=\"row\"><div class=\"columns large-6 large-centered\"><p>Built with [Wintersmith](http://wintersmith.io), [Jade](http://jade-lang.com), [GruntJs](http://gruntjs.com), [Foundation Zurb](http://foundation.zurb.com), [Bourbon](http://bourbon.io), [AngularJs](http://angularjs.org/), [requirejs](http://requirejs.org), [lodash](http://underscorejs.org), [Modernizr](http://modernizr.com/) and [SublimetText3](https://sublimetext.com/).</p><p>DAYZ is a registered trademark of [Bohemia Interactive a.s](href=\"www.bistudio.com/‎\"). This is an unofficial modification that is not affiliated or authorized by Bohemia Interactive a.s.</p><p>DayZ Epoch is licensed under [DayZ Mod License Share Alike](href=\"http://www.bistudio.com/licenses/dayz-mod-license-share-alike\").</p></div></div>");
}]);

angular.module("/epochdb/js/templates/partial/item-tabs.html", []).run(["$templateCache", function($templateCache) {
$templateCache.put("/epochdb/js/templates/partial/item-tabs.html",
"<dl class=\"tabs simple-outline\"><dd data-ng-repeat=\"tab in Tabs\"><a title=\"{{ tab.name }}\" data-ng-click=\"tab.toggle()\"><em data-ng-class=\"['icon-{{ tab.name }}']\" class=\"item-detail-tab\"></em></a></dd></dl><div data-ng-show=\"Visible\" class=\"tabs-content\"><div data-ng-repeat=\"pane in Panes\" data-item-summary-pane=\"{{pane}}\" data-collection=\"{{ pane.data }}\" class=\"content\"></div></div>");
}]);
...
"<li data-ng-class=\"['type-'+item.type.toLowerCase(), isVisible?'active':'']\" data-ng-click=\"isVisible=!isVisible\" class=\"epoch-item-summary\"><a title=\"{{ item.id }}\" class=\"button text-left\"><span class=\"label radius epoch-type-icon\"><em data-ng-class=\"'icon-' + item.type.toLowerCase()\"></em></span><span data-ng-if=\"item.count\" class=\"epoch-item-count label\">{{ item.count }}</span><span data-ng-bind=\"item.name\" class=\"epoch-item-name\"></span></a></li>");
}]);
...
$templateCache.put("/epochdb/js/templates/partial/loading.html",
"<div id=\"loading\" class=\"alert\">Loading...</div>");
}]);
			...
			function($parse, AssetManager){
				...
				return {
					...
					link: function($scope, $element, $attrs){
						...
						_.forEach($scope.item.depends, function(value, key){
							$scope.Panes.push({
								name: key,
								data: $scope.item.depends[key]
							})
						});
						...
						$scope.$on("summary-toggled", function(scope, id){
						});
